...
class _PagoScreenState extends State<PagoScreen> {
  ...
  final metodos = [
    {'id': 'tarjeta', 'label': 'Tarjeta (Stripe)', 'icono': Icons.credit_card},
  ...
  Future<void> _pagar() async {
    ...
    // Si el método es tarjeta, usar Stripe
    if (metodoPago == 'tarjeta') {
      await _pagarConStripe();
    } else {
      await _pagarSinStripe();
    }
  }
  ...
  Future<void> _pagarConStripe() async {
    try {
      ...
      final intentData = await PagoService.crearPaymentIntent(
      ...
      if (intentData == null) {
        setState(() {
          error = 'Error al iniciar el pago. El incidente puede ya tener un pago registrado.';
          cargando = false;
        });
        return;
      }
      ...
      // 2. Mostrar formulario de pago de Stripe
      ...
      final res = await PagoService.confirmarPago(
        incidenteId: widget.incidenteId,
        montoTotal: monto,
        metodoPago: metodoPago,
      ...
      if (res != null) {
        setState(() {
          exito = '✅ Pago realizado correctamente con Stripe';
          cargando = false;
        });
        ...
        setState(() {
          error = 'Error al confirmar el pago en el servidor.';
          cargando = false;
        });
      }
      ...
      setState(() {
        ...
        if (e.error.code == FailureCode.Canceled) {
          ...
          error = 'Error de Stripe: ${e.error.localizedMessage ?? e.error.message}';
        }
        cargando = false;
      });
    } catch (e) {
  ...
  Future<void> _pagarSinStripe() async {
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (monto > 0) ...[
                    ...
                    _filaResumen(
                      'Taller recibe',
                      'Bs. ${totalTaller.toStringAsFixed(2)}',
            ...
            if (metodoPago == 'tarjeta') ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: const Color(0xFFBA7517).withOpacity(0.4)),
                ),
                child: const Row(
                  children: [
                    Icon(Icons.info_outline, color: Color(0xFFBA7517), size: 16),
                    SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Modo test activo. Usá la tarjeta 4242 4242 4242 4242, cualquier fecha futura y cualquier CVC.',
            ...
            ElevatedButton.icon(
              ...
              icon: cargando
                  ...
                  : Icon(metodoPago == 'tarjeta' ? Icons.lock : Icons.check),
              label: Text(
                cargando
                    ? 'Procesando...'
                    : 'Confirmar pago${monto > 0 ? ' · Bs. ${monto.toStringAsFixed(2)}' : ''}',
